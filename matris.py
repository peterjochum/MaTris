...
class Matris(object):
    ...
    def place_shadow(self):
        ...
        """
        pos_y, pos_x = self.tetromino_position
        while self.blend(position=(pos_y, pos_x)):
            pos_y += 1

        position = (pos_y - 1, pos_x)
    ...
    def fits_in_matrix(self, shape, position):
        ...
        for x in range(pos_x, pos_x + len(shape)):
            for y in range(pos_y, pos_y + len(shape)):
                if (
                    self.matrix.get((y, x), False) is False
                    and shape[y - pos_y][x - pos_x]
                ):  # outside matrix
                    return False
    ...
    def request_movement(self, direction):
        ...
        """
        pos_y, pos_x = self.tetromino_position
        if direction == "left" and self.blend(position=(pos_y, pos_x - 1)):
            self.tetromino_position = (pos_y, pos_x - 1)
            self.needs_redraw = True
            return self.tetromino_position
        elif direction == "right" and self.blend(position=(pos_y, pos_x + 1)):
            self.tetromino_position = (pos_y, pos_x + 1)
            self.needs_redraw = True
            return self.tetromino_position
        elif direction == "up" and self.blend(position=(pos_y - 1, pos_x)):
            self.needs_redraw = True
            self.tetromino_position = (pos_y - 1, pos_x)
            return self.tetromino_position
        elif direction == "down" and self.blend(position=(pos_y + 1, pos_x)):
            self.needs_redraw = True
            self.tetromino_position = (pos_y + 1, pos_x)
            return self.tetromino_position
        else:
        ...
        for x in range(pos_x, pos_x + len(shape)):
            for y in range(pos_y, pos_y + len(shape)):
                if (
                    ...
                    and shape[y - pos_y][x - pos_x]  # shape is outside the matrix
...
class Menu(object):
    """
    Creates main menu which allows new games quitting and viewing the high score
    """
    ...
    def main(self, screen):
        ...
        highscore_surf = self.construct_highscoresurf()  # Loads highscore onto menu
        ...
        while self.running:
            ...
            if time_passed > 1:  # A game has most likely been played
                highscore_surf = self.construct_highscoresurf()

            screen.blit(nightmare, (0, 0))
            screen.blit(
                highscore_surf,
                highscore_surf.get_rect(right=WIDTH - 50, bottom=HEIGHT - 50),
            )
            menu.draw(screen)
            pygame.display.flip()
...
def construct_nightmare(size):
    ...
    boxsize = 8
    bordersize = 1
    vals = "12345"  # only the lower values, for darker colors and greater fear
